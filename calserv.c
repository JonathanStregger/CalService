...
int main(int argc, char **argv){
  ...
  clientData = loadCalendarData(filename);
...
void openConnection(char *port, calendar *caldata){
  ...
  if((aptfd = accept(sockfd, (struct sockaddr *)&client_addr,
      &addr_size)) == -1){
  ...
  while(1){
    ...
    while(strstr(request, "\n\n") == NULL && ttlSR < 255){
      if((bytesSR = recv(aptfd, reqPt, buffSize, 0)) <= 0){
        fprintf(stderr, "No longer receiving from client. "
    ...
    switch(parseCommand(request, caldata)){
      ...
      case 7:
        ...
        break;
      case 8:
        reply = getRequest(request, caldata);
        break;
      default:
...
calendar* loadCalendarData(char * filename){
  printf("Loading calendar from file %s\n", filename);
 ...
 *          500 appointment retrieved.  If appointment cannot be retieved,
...
char * getRequest(char *request, calendar *cal){
  ...
  //if only a date, should be yy-mm-dd\n\n
    ...
    if(searchApp == NULL){
      strcpy(reply, "504 apppointment in wrong format\n");
      return reply;
    }
    else{
      for(i = 0; i < cal->added; i++){
...
int saveCalendarData(calendar *calData){
  ...
  //if there are no appointments to save, do not do a save.
  if(calData->added == 0){
    printf("No appointments available to save.\n");
    return 7;
  }

  //check if the calendar file exists yet.
  //FILE *oldCal = fopen(calData->filename, "r");
  //if(oldCal != NULL){
  if( access( calData->filename, F_OK ) != -1 ) {
    //fclose(oldCal);
    ...
    FILE *calfile = fopen(calData->filename, "w");
    if(calfile == NULL){
      printf("Error creating new calFile. No changes made.\n");
      return 7;
    }
    ...
    if(prevOpt != 0){
      switch(prevOpt){
        ...
        *portNumber = atoi(argv[i]);
        ...
        //set filename option
        case 'f':
          //max unix filename length is generally 255
